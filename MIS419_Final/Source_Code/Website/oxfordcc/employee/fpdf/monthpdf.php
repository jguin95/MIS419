   ...
   include_once($path);
			...
			try {
				...
				$STH = $dbc->prepare("SELECT Jobs.JobName AS job, DV.AH AS hours, DV.SW AS SUM_WAGE, MNUMS.SM AS SUM_MAT
							...
							SELECT MONTH( DATE ) AS MD, JobID, SUM( Wages ) AS SW, SUM( Approved_hrs ) AS AH
							FROM Hours
							GROUP BY JobID, MD
							HAVING MD = :month
							)DV LEFT OUTER JOIN (
							
							SELECT MONTH( MPDate ) AS MPD, JobNum, SUM( MCost ) AS SM
							FROM Materials
							GROUP BY Jobnum, MPD
							HAVING MPD = :month
...
IF($rows == 0){
	$pdf->Cell(0,10,'No Data Found',0,1);
 	...
 	$pdf->Cell(0,10,'Total Expenditures for '.$monthName.' is '.$total.' dollars.' ,0,1);
}
$pdf->Output();
?>